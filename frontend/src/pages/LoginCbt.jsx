...
export default function LoginCbt() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || t('cbt_error'));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-900 flex justify-center items-center relative overflow-hidden px-4">
      ...
      <div className="glass-dark p-10 rounded-3xl max-w-md w-full relative z-10 border border-slate-700 shadow-2xl fade-in text-center">
        ...
        <h2 className="text-3xl font-black text-white tracking-tight uppercase">{t('cbt_gate')}</h2>
        <p className="text-slate-400 text-sm mt-2 mb-8">{t('cbt_subtitle')}</p>
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          <div>
            <label className="block text-sm font-bold text-slate-300 mb-2 uppercase tracking-widest text-left">{t('cbt_token_label')}</label>
            <input 
              type="text" 
              value={token}
              onChange={(e) => setToken(e.target.value.toUpperCase())}
              placeholder={t('cbt_token_placeholder')}
              maxLength={6}
          ...
          >
            {loading ? t('cbt_btn_verifying') : t('cbt_btn_start')}
